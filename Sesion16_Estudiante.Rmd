...
```{r include=FALSE}
library(dplyr)
estudiante2 = estudiantes1
estudiante2 = estudiante2 %>% select("Año y periodo", "Programa de estudio", "Sexo", "Ciudad de colegio", "Biología", "Química", "Física", "Matemáticas", "Filosofía", "Historia", "Lenguaje", "Sociales", "Idioma","Interdiciplinar")
```
...
```{r include=FALSE}
estudiante2 = estudiante2 %>% rename(Periodo = "Año y periodo", Programa = "Programa de estudio", Genero = Sexo, Ciudad = "Ciudad de colegio")
```
...
```{r include=FALSE}
estudiante2$Ciudad = as.factor(estudiante2$Ciudad)
levels(estudiante2$Ciudad)
```
...
```{r echo=FALSE}
library(knitr)
knitr::kable(head(estudiante2),caption = "Tabla final para graficar")
```
